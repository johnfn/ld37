...
[RequireComponent(typeof(ControllableStats))]
[DisallowMultipleComponent]
public class PhysicsController2D : Entity
{
    ...
    void Update()
    {
        ...
        if (Collisions.TouchingBottom) {
            _velocity.y = Math.Max(_velocity.y, 0);
        }

        if (Collisions.TouchingTop) {
    ...
    void CheckForCollisions(ref Vector3 unscaledVelocity)
    {
        // Check for vertical collisions
        const int numRays = 8;
        const float skinWidth = .01f;
        var velocity = unscaledVelocity * Time.deltaTime;
